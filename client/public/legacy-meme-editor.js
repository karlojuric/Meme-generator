;(function () {
    if (window.__memeEditorLoaded) {
        console.warn('[meme-editor] script already initialized');
        return;
    }
    window.__memeEditorLoaded = true;

    // Canvas and context
    const canvas = document.getElementById('memeCanvas');
    if (!canvas) {
        console.warn('[meme-editor] canvas element not found, aborting init');
        window.__memeEditorLoaded = false;
        return;
    }
...
function exportCanvasBlob(type = 'image/png', encoderOptions = 0.92) {
    return new Promise((resolve, reject) => {
        canvas.toBlob((blob) => {
            if (!blob) {
                reject(new Error('Unable to export meme canvas'));
                return;
            }
            resolve(blob);
        }, type, encoderOptions);
...
window.memeEditor = {
    ...
    getState: serializeEditorState,
    exportBlob: exportCanvasBlob,
    hasBackground: () => Boolean(backgroundImage),
    reset: resetEditor,
    addTextBox: addDescription,
    loadState: async ({ imageUrl, canvasState }) => {
        try {
            if (imageUrl) {
            ...
            if (canvasState) {
                const parsed =
                    typeof canvasState === 'string'
                        ...
                        : canvasState;
                if (parsed?.textBoxes) {
                    textBoxes = parsed.textBoxes.map(tb => ({ ...tb }));
                    nextTextBoxId =
                        textBoxes.reduce(
                            (max, tb) => Math.max(max, tb.id || 0),
                            0,
                        ) + 1;
                } else {
                    textBoxes = [];
            ...
            console.error('Failed to load meme state', error);
